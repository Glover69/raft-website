...
const Hero: React.FC<HeroProps> = ({ onAnimationStart }) => {
  useEffect(() => {
    if (onAnimationStart) {
      ...
      // gsap.to(heroTextMain, {
      //   x: 400,
      //   ease: 'power4.in',
      //   duration: 3,
      //   scrollTrigger: {
      //     trigger: heroTextMain,
      //     markers: true,
      //     start: "top bottom",
      //     end: "bottom top",
      //     scrub: 1
  ...
  return (
    <div className="main-hero-wrapper flex flex-col items-center justify-center px-6 md:px-10 3xl:px-12 py-24 gap-20">
      <div
        style={divStyle}
        className="hero-upper opacity-0 flex flex-col items-center gap-4"
      >
        <div className="card-raft opacity-0 flex flex-row items-center gap-2 rounded-[6rem] py-2 px-4 border-[0.2px] border-[#989898] bg-[#FFFFFF26] backdrop-blur-[10px]">
          <span className="SFRegular text-[#DCDCDC]">
        ...
        <div className="flex flex-col items-center gap-4 overflow-hidden w-fit h-fit">
          <h1 className="SFRegular opacity-0 main-hero-text text-4xl md:text-6xl lg:text-7xl 3xl:text-9xl md:w-[40rem] 3xl:w-[65rem] 3xl:py-4 lg:leading-[5rem] text-center overflow-hidden">
            Building the future of banking.
          </h1>
        </div>

        <span className="SFRegular opacity-0 hero-subtitle text-[#BDBDBD] md:w-[35rem] md:text-2xl lg:leading-9 text-center">
          Experience the future of banking with RAFT. We&apos;re here to empower
          your financial journey.
        </span>

        <button className="bg-[#2B892E] SFMedium rounded-[6rem] px-8 py-4 3xl:px-10 3xl:py-5 2xl:text-xl opacity-0">
          Get Started
        </button>
      </div>
      ...
      <div
        style={heroImage}
        className="h-[24rem] lg:h-[35rem] xl:h-[45rem] 3xl:h-[55rem] w-0 rounded-lg opacity-0 hero-image"
      >
        <div style={innerBG} className="inner w-full h-full"></div>
      </div>
    </div>
  );
};
